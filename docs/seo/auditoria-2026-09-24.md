...
7. ~~Códigos AFIP/ARCA~~ **Resuelto con el listado de Darío** (RNAS de la SSSalud, verificado el 20-sep-2026): código en la ficha de 14 prepagas (todas menos Premedic) y en 15 fichas de obras sociales (en otras 14, provinciales y de régimen propio, se explica por qué no tienen), más el directorio `/obras-sociales/codigos` con 398 entidades. **Falta Premedic**: no está en el listado ("PREMEDICA S.A.", 9-0420-3, es otra empresa, de Catamarca).
11. **OSPAT**: en el RNAS es la obra social del personal **del turf** (1-2170-5) y la ficha `/obras-sociales/ospat` la presenta como de telecomunicaciones. Revisar la ficha; mientras tanto no muestra código.
12. **Ancho de las páginas**: la regla `.container` de `app/globals.css` (max-width 1200px) pisa las clases `max-w-*` de Tailwind, así que las páginas escritas como `container max-w-4xl` salen a 1200px en desktop (líneas de texto muy largas). Las páginas nuevas usan `max-w-3xl!`. Arreglarlo en todo el sitio cambia el ancho de muchas páginas: conviene revisarlo con capturas antes.
