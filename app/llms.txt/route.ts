...
export function GET() {
  ...
  const lineas = [
    ...
    '',
    '## Qué prepagas atienden en cada sanatorio (CABA, GBA e interior)',
    ...
    ...sanatoriosPublicables().map((s) => `- [${s.nombre}${s.ciudadNombre && !s.nombre.includes(s.ciudadNombre) ? ` (${s.ciudadNombre})` : ''}](${SITE_URL}/sanatorios/${s.slug}): ${prepagasEnSanatorio(s.slug).map((p) => p.desde ? `${p.prepagaNombre} desde ${p.desde.label}` : `${p.prepagaNombre} (guardia)`).join('; ')}`),
    '',
    '## Trámites',
    ...
    '',
    '## Obras sociales',
    `- [PAMI: teléfono 138, medicamentos y afiliación](${SITE_URL}/obras-sociales/pami)`,
    `- [IOMA: obra social de la provincia de Buenos Aires](${SITE_URL}/obras-sociales/ioma)`,
    `- [OSECAC: teléfonos, turnos y guardias](${SITE_URL}/obras-sociales/osecac)`,
    `- [OSPe (Obra Social de Petroleros)](${SITE_URL}/obras-sociales/ospe)`,
    `- [OSPRERA (trabajadores rurales)](${SITE_URL}/obras-sociales/osprera)`,
